...
interface EditStudentModalProps {
  ...
  currentUserRole: 'student' | 'professor';
}

const EditStudentModal: React.FC<EditStudentModalProps> = ({ isOpen, onClose, student, onSave, currentUserRole }) => {
  const { t } = useLanguage();
  ...
  const handleSave = () => {
    if (validate()) {
      ...
      if (newPassword) {
        finalStudentData.password = newPassword;
      }
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md animate-scaleIn" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-xl font-bold">{currentUserRole === 'professor' ? t.dashboard.editStudent : t.profile.editProfile}</h2>
        </header>
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <select name="class" id="class" value={formData.class} onChange={handleChange} disabled={currentUserRole === 'student'} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm dark:bg-slate-700 dark:border-slate-600 disabled:bg-slate-200 dark:disabled:bg-slate-700/50">
           ...
           <div>
            ...
            <input type="number" name="number" id="number" value={formData.number} onChange={handleChange} disabled={currentUserRole === 'student'} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm dark:bg-slate-700 dark:border-slate-600 disabled:bg-slate-200 dark:disabled:bg-slate-700/50" />
            {errors.number && <p className="text-red-500 text-xs mt-1">{errors.number}</p>}
          </div>
          <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
            <label htmlFor="newPassword" className="block text-sm font-medium text-slate-700 dark:text-slate-300">{t.profile.changePassword}</label>
            <input type="password" name="newPassword" id="newPassword" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder={t.profile.newPassword} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm dark:bg-slate-700 dark:border-slate-600" />
